...
def inventory(to_screen=True, to_file=True, filename='inventory_result.json'):
    ...
    inventory['os_hostname'] = subprocess.run(['hostname'], capture_output=True, text=True).stdout.splitlines()[0]

    inventory['os_version'] = subprocess.run(['grep', '-i', 'pretty', '/etc/os-release'],
                                             capture_output=True, text=True
                                             ).stdout.split("\"")[1]
    inventory['os_core'] = subprocess.run(['uname', '-r'], capture_output=True, text=True).stdout.splitlines()[0]
    ...
    for rec in netstat:
        ...
        rec.append(rec[3].replace(f':{rec[-1]}', ''))

    netstat = sorted(netstat, key=itemgetter(-1))
    netstat = sorted(netstat, key=itemgetter(-2))

    for rec in netstat:
        id = f'port {rec[0]}/{rec[-2]} on {rec[-1]}'
        inventory['os_listen_ports'][id] = {}
        inventory['os_listen_ports'][id]['listen_proto'] = rec[0]
        inventory['os_listen_ports'][id]['listen_port'] = str(rec[-2])
        inventory['os_listen_ports'][id]['listen_ip'] = rec[-1]
        inventory['os_listen_ports'][id]['listen_pid_program'] = rec[-3]

    inventory['os_ssh_port'] = subprocess.run(['grep', '-i', 'port', '/etc/ssh/sshd_config'],
                                              capture_output=True, text=True
                                              ).stdout.splitlines()[0].split()[1]

    inventory['os_ssl_version'] = subprocess.run(['openssl', 'version'], capture_output=True, text=True).stdout.splitlines()[0]
    
    temp_ssh_version1 = subprocess.run(['ssh', '-V'], capture_output=True, text=True).stdout
    temp_ssh_version2 = subprocess.run(['ssh', '-V'], capture_output=True, text=True).stderr

    if len(temp_ssh_version1) == 0:
        inventory['os_ssh_version'] = temp_ssh_version2.splitlines()[0]
    else:
        inventory['os_ssh_version'] = temp_ssh_version1.splitlines()[0]
    ...
    elif os.path.exists('/opt/MegaRAID/storcli/storcli'):
        storcli_path = '/opt/MegaRAID/storcli/storcli64'
    else:
        storcli_path = None

    if storcli_path is not None:
        parse_storcli_l0(json.loads(subprocess.run([storcli_path, '/call', '/eall', '/sall', 'show', 'all', 'J'], capture_output=True, text=True).stdout), inventory)
